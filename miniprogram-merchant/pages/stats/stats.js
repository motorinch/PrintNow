...
Page({
  ...
  buildChart(daily) {
    ...
    return daily.map((d, i) => ({
      ...
      value: d.count,
      height: Math.max(10, (d.count / maxVal) * 150),
      color: colors[i] || '#4A90D9'
    }));
  },

  buildStatusDist(items) {
    const colorMap = { '待支付': '#E65100', '已支付': '#1565C0', '打印中': '#2E7D32', '待取件': '#C62828', '已完成': '#27AE60', '已取消': '#95A5A6' };
    const total = items.reduce((s, i) => s + i.count, 0) || 1;
    return items.map(i => ({
      ...i,
      pct: Math.round(i.count / total * 100),
      color: colorMap[i.label] || '#999'
    }));
  }
});
